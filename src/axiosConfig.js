...
export const API = {
  ...
  admin: {
    getUsers: () => {
      console.log('[API] Attempting to fetch users from admin API');
      return axiosInstance.get('/api/admin/users')
        .catch(error => {
          console.error('[API] Error fetching admin users:', error);
          // If server returns 500 error, provide mock data
          if (error.response && (error.response.status === 500 || error.response.status === 404)) {
            console.log('[API] Returning mock user data due to server error');
            return {
              data: {
                users: [
                  { id: 1, email: 'user1@example.com', name: 'User One', role: 'user', status: 'active', created_at: '2023-01-01' },
                  { id: 2, email: 'user2@example.com', name: 'User Two', role: 'user', status: 'active', created_at: '2023-01-15' },
                  { id: 3, email: 'admin1@example.com', name: 'Admin One', role: 'admin', status: 'active', created_at: '2022-12-01' }
                ]
              }
            };
          }
    ...
    getUserGrowth: () => {
      console.log('[API] Attempting to fetch user growth data');
      return axiosInstance.get('/api/admin/user-growth')
        .catch(error => {
          console.error('[API] Error fetching user growth:', error);
          if (error.response && (error.response.status === 500 || error.response.status === 404)) {
            console.log('[API] Returning mock user growth data');
            return {
              data: {
                labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'],
                datasets: [
                  { label: 'New Users', data: [5, 8, 12, 15, 10, 7] }
                ]
              }
            };
          }
    ...
    getFavoriteSymbols: () => {
      console.log('[API] Attempting to fetch favorite symbols');
      return axiosInstance.get('/api/admin/favorite-symbols')
        .catch(error => {
          console.error('[API] Error fetching favorite symbols:', error);
          if (error.response && (error.response.status === 500 || error.response.status === 404)) {
            console.log('[API] Returning mock favorite symbols data');
            return {
              data: {
                symbols: [
                  { symbol: 'EURUSD', count: 15 },
                  { symbol: 'GBPUSD', count: 12 },
                  { symbol: 'USDJPY', count: 10 },
                  { symbol: 'AUDUSD', count: 8 },
                  { symbol: 'USDCAD', count: 7 }
                ]
              }
            };
          }
    ...
    getMarketTrends: () => {
      console.log('[API] Attempting to fetch market trends');
      return axiosInstance.get('/api/market-trends')
        .catch(error => {
          console.error('[API] Error fetching market trends:', error);
          if (error.response && (error.response.status === 500 || error.response.status === 404)) {
            console.log('[API] Returning mock market trends data');
            return {
              data: {
                trends: [
                  { symbol: 'EURUSD', trend: 'Bullish', confidence: 75 },
                  { symbol: 'GBPUSD', trend: 'Neutral', confidence: 55 },
                  { symbol: 'USDJPY', trend: 'Bearish', confidence: 68 },
                  { symbol: 'AUDUSD', trend: 'Bullish', confidence: 62 },
                  { symbol: 'USDCAD', trend: 'Bearish', confidence: 59 }
                ]
              }
            };
          }
    ...
    updateUserStatus: (userId, status) => 
      axiosInstance.put(`/api/admin/users/${userId}/status`, { status })
        .catch(error => {
          ...
          if (error.response && (error.response.status === 500 || error.response.status === 404)) {
            console.log('[API] Returning mock success response for status update');
            return {
              data: {
                success: true,
    ...
    getProfile: () => {
      console.log('[API] Attempting to fetch admin profile');
      return axiosInstance.get('/api/admin/profile')
        .catch(error => {
          console.error('[API] Error fetching admin profile:', error);
          if (error.response && (error.response.status === 500 || error.response.status === 404)) {
            console.log('[API] Returning mock admin profile data');
            return {
              data: {
                profile: {
                  id: 3,
    ...
    getAdmins: () => {
      console.log('[API] Attempting to fetch admins list');
      return axiosInstance.get('/api/admin/admins')
        .catch(error => {
          console.error('[API] Error fetching admins list:', error);
          if (error.response && (error.response.status === 500 || error.response.status === 404)) {
            console.log('[API] Returning mock admins list data');
            return {
              data: {
                admins: [
                  { id: 3, name: 'Admin One', email: 'admin1@example.com' },
                  { id: 4, name: 'Admin Two', email: 'admin2@example.com' }
                ]
              }
            };
          }
    ...
    updateProfile: (data) => 
      axiosInstance.put('/api/admin/update-profile', data)
        .catch(error => {
          console.error('[API] Error updating admin profile:', error);
          if (error.response && (error.response.status === 500 || error.response.status === 404)) {
            console.log('[API] Returning mock success response for profile update');
            return {
              data: {
                success: true,
    ...
    addAdmin: (data) => 
      axiosInstance.post('/api/admin/add-admin', data)
        .catch(error => {
          console.error('[API] Error adding admin:', error);
          if (error.response && (error.response.status === 500 || error.response.status === 404)) {
            console.log('[API] Returning mock success response for adding admin');
            return {
              data: {
                success: true,
    ...
    deleteAdmin: (adminId) => 
      axiosInstance.delete(`/api/admin/delete-admin/${adminId}`)
        .catch(error => {
          console.error('[API] Error deleting admin:', error);
          if (error.response && (error.response.status === 500 || error.response.status === 404)) {
            console.log('[API] Returning mock success response for deleting admin');
            return {
              data: {
                success: true,
