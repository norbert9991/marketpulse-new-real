                ...
                <Grid container spacing={4} sx={{ mt: 2 }}>
                  <Grid item xs={12} md={6}>
                    <Paper sx={{ 
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                        ...
                        <Chip 
                          label={`${simulationResults.symbols.length} pairs`} 
                          size="small" 
                          sx={{ 
                            backgroundColor: colors.accentBlue, 
                            color: colors.primaryText 
                      ...
                      <Box sx={{ height: 240, position: 'relative' }}>
                        ...
                        {/* Legend */}
                        <Box sx={{ position: 'absolute', top: 0, right: 0, maxHeight: 240, overflow: 'auto', pr: 2 }}>
                          {simulationResults.symbols.map((symbol, i) => {
                            const chartColors = [
                              '#00E676',
                              '#2196F3',
                            ...
                            return (
                              <Box key={symbol.symbol} sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                                ...
                                />
                                <Typography variant="caption" sx={{ mr: 1, color: colors.primaryText }}>
                                  {symbol.symbol}
                                </Typography>
                  ...
                  <Grid item xs={12} md={6}>
                    <Paper sx={{ 
                      ...
                      {/* Bar chart styled like Monthly P/L */}
                      <Box sx={{ height: 210, mb: 2 }}>
                        <svg width="100%" height="210" preserveAspectRatio="none">
                          <g>
                            {simulationResults.symbols.map((symbol, i) => {
                              const maxProfit = Math.max(...simulationResults.symbols.map(s => s.pl));
                              const barHeight = (symbol.pl / maxProfit) * 140;
                              const barWidth = Math.min(60, (100 / simulationResults.symbols.length) - 10);
                              const totalWidth = simulationResults.symbols.length * (barWidth + 10);
                              const spacing = 10;
                      ...
                      <Box sx={{ 
                        ...
                        <Box>
                          ...
                          <Typography variant="h6" sx={{ color: colors.primaryText, fontWeight: 'bold' }}>
                            {simulationResults.symbols.reduce((prev, current) => 
                              (prev.pl > current.pl) ? prev : current
                            ).symbol}
                          </Typography>
                        </Box>
                        ...
                        <Box sx={{ textAlign: 'right' }}>
                          ...
                          <Typography variant="h6" sx={{ color: '#00E676', fontWeight: 'bold' }}>
                            +${(simulationResults.symbols.reduce((sum, s) => sum + s.pl, 0) / 
                              simulationResults.symbols.length).toFixed(2)}
                          </Typography>
                        </Box>
                      </Box>
                    </Paper>
                        ...
                        {simulationResults.insights.map((insight, index) => (
                          <Box 
                            ...
                            sx={{ 
                              ...
                              mb: index < simulationResults.insights.length - 1 ? 2 : 0,
                              pl: 2,
                              borderLeft: `3px solid ${
                                insight.includes('best performing') ? '#00E676' : 
                                insight.includes('achieved') && !insight.includes('not') ? '#2196F3' : 
                                '#FF9800'
                              }`
                            }}
                          >
